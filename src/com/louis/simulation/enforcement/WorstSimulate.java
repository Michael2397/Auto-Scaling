...
public class WorstSimulate {
	...
	public static ArrayList<Integer> nextIntervalRequestlist5 ;//从预测文件中读取
...
//-------------------------------------------start:仿真MAPE----------------------------------------------------------------//	
        ...
        while(time<=Constants.terminateTime) {
			...
			if(time%Constants.schedulingInterval==0&&time!=0) {
				...
				//1、最坏的情况：知道cpu后才进行调度,而且要有2分钟的开机时间，实现方式是结合isAddVM，createVm()*/
				float cpuU =  Constants.cpuUtilizationList300.get(Constants.time300-1);
				System.out.println("####虚拟机需要调度啦！####");
				//虚拟机调度
				execute(cpuU); 
...
//--------------------------------------------end:结束打印---------------------------------------------------------------//
       /*excel中的time时间列*/ 
		...
		ArrayListToFile.arrayListToFile(Constants.numberOfRequest5, "F:/VmExperiment/numberOfRequest5/numberOfRequest5.txt");
		ArrayListToFile.arrayListToFile(Constants.vmNumAll, "F:/VmExperiment/vmNumAllTxt/worst.txt");
		ArrayListToFile.arrayListToFile3(Constants.cpuUtilizationList300, "F:/VmExperiment/CPUTxt/worst.txt");
		ArrayListToFile.arrayListToFile(Constants.slaTotalTime, "F:/VmExperiment/SLATxt/worst.txt");
...
//--------------------------------------------start:vm调度部分---------------------------------------------------------------//
	...
	/*
	 * 最坏的调度方案:直接使用这段300秒内的cpu利用率进行调度
	 * */
	private static void execute(float f) {
		if(f>0.7) {
			System.out.println("增加虚拟机");
        	int addVmNum = 1;
        	isAddVM = true;
        	createVm(time, addVmNum);
        	executeVmCount++;
		}else if(f<0.3) {
			System.out.println("删除虚拟机");
        	int deleteVmNum = 1;
        	executeVmCount++;
        	deleteVm(deleteVmNum);
		}
	}
