...
public class LinearSimulate {
	...
	public static int[] linearPredictValue = LinearRegression.getLinearPredict();
...
//-------------------------------------------start:仿真MAPE----------------------------------------------------------------//	
        ...
        while(time<=Constants.terminateTime) {
			...
			if(time%Constants.schedulingInterval==0&&time!=0) {
				...
				float predictValue1 = linearPredictValue[nextPredictTime];
				nextPredictTime++;
				//虚拟机调度
...
//--------------------------------------------end:结束打印---------------------------------------------------------------//
       /*excel中的time时间列*/ 
		...
		ArrayListToFile.arrayListToFile(Constants.vmNumAll, "F:/VmExperiment/vmNumAllTxt/linear.txt");
		ArrayListToFile.arrayListToFile3(Constants.cpuUtilizationList300, "F:/VmExperiment/CPUTxt/linear.txt");
		ArrayListToFile.arrayListToFile(Constants.slaTotalTime, "F:/VmExperiment/SLATxt/linear.txt");
